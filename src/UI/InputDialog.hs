...
import Brick.Widgets.Core (vBox, str, hLimit, vLimit, withBorderStyle, padLeftRight, padTop, padTopBottom, withAttr, updateAttrMap)
...
inputDialogUI editorState label helpString =
  let ui = vCenter
         ...
         $ borderWithLabel (withAttr inputDialogLabelAttr $ padLeftRight 1 $ str label)
